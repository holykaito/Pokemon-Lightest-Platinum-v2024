...
class RaidScene
  ...
  def pbSavingPrompt(rules)
    ...
    if !raid_pkmn
      ...
      if save_game
        ...
        pbSEPlay("GUI save choice")
        @interp.setVariable(rules.clone)
        if Game.save
  ...
  #-----------------------------------------------------------------------------
  def pbStartScene(rules)
    @sprites    = {}
    @viewport   = Viewport.new(0, 0, Graphics.width, Graphics.height)
    @viewport.z = 99999
    $PokemonGlobal.nextBattleBGM = nil
    return false if !pbSavingPrompt(rules)
    @rules = rules
    @rules[:pokemon] = (@interp.getVariable.is_a?(Hash)) ? @interp.getVariable[:pokemon] : rules[:pokemon]
    @rules[:pokemon].heal
    @pkmn = @rules[:pokemon].clone
    ...
    if battleRules["defaultWeather"] && battleRules["defaultWeather"] != :None
      @weather = battleRules["defaultWeather"].clone
    else
    ...
    if battleRules["defaultTerrain"] && battleRules["defaultTerrain"] != :None
      @terrain = battleRules["defaultTerrain"].clone
    else
      @terrain = :None
    ...
    @environ = battleRules["environment"].clone || pbGetEnvironment
        ...
        when 0 # Begin Raid
          ...
          if pbConfirmMessage(_INTL("Enter the raid with the displayed {1}?", party_display))
            ...
            pbFadeOutIn {
              ...
              pbDisposeSpriteHash(@sprites)
              @viewport.dispose
              outcome = WildBattle.start_core(@rules[:pokemon])
              pbWait(0.5)
              pbSEPlay("Door exit")
            }
...
class RaidScreen
  ...
  def pbStartScreen(rules)
    outcome = @scene.pbStartScene(rules)
    @scene.pbEndScene
    return outcome
  end
end

def pbRaidDenEntry(rules)
  scene  = RaidScene.new
  screen = RaidScreen.new(scene)
  return screen.pbStartScreen(rules)
end