...
class RaidAdventureState
  ...
  def endAdventure?
    return true if @outcome > 0
    return false if endlessMode?
    return true if @boss_battled
    return false
  end
  ...
  def log_endless_record
    ...
    if !record || record.empty? || @floor > record[:floor]
      $player.party.each { |p| p.heal }
        record[:map]     = @map.id
        record[:floor]   = @floor
        record[:battles] = @battle_count
        record[:party]   = []
        $player.party.each do |pkmn|
        record[:party].push(pkmn.clone)
      end
    end
  end
end
...
#===============================================================================
# Raid Adventure calls used for initiating an Adventure.
#-------------------------------------------------------------------------------
...
class RaidAdventure
  #-----------------------------------------------------------------------------
  # Main Adventure call. Displays intro and outro text and selections.
  #-----------------------------------------------------------------------------
  def self.start(advData = {})
    ...
    if advData[:boss]
      if GameData::Species.exists?(advData[:boss])
  ...
  #-----------------------------------------------------------------------------
  # Core Adventure call. Enters and exits the Adventure with no text or selections.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Adventure intro text and selections.
  #-----------------------------------------------------------------------------
  def self.adventure_intro(advData)
    ...
    if advData[:endless]
      ...
      loop do
        ...
        when 2 # View Record
          pbAdventureMenuRecord(advData[:style])
        else # Nevermind
          break
        end
  ...
  #-----------------------------------------------------------------------------
  # Adventure outro text and selections.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Generates all species to be found in this Adventure's lair.
  #-----------------------------------------------------------------------------
  def self.generate_raid_species(species, style)
    ...
    battle_count.times do |i|
      ...
      sp = raid_species[rank].last if sp.nil? && raid_species[rank]
      sp = :DITTO if sp.nil?
      raidRanks.each_key { |key| raidRanks[key].delete(sp) }
  ...
  #-----------------------------------------------------------------------------
  # Generates the party the player will use during this Adventure.
  #-----------------------------------------------------------------------------
...
class PokemonGlobalMetadata
  #-----------------------------------------------------------------------------
  # Aliased to initialize various Raid Adventure properties.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Related to the current Raid Adventure state.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Returns saved routes from previous Raid Adventures.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Related to unlocking Endless Mode.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Returns the saved record from the furthest journey in Endless Mode.
  #-----------------------------------------------------------------------------
...
#===============================================================================
# For obtaining the status of the current Raid Adventure stat.
#===============================================================================
