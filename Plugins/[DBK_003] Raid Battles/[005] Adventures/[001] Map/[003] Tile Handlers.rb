...
AdventureTileEffects::MapTile.add(:RandomTurn,
  proc { |id, tile, adventure, scene, dir, dirs|
    case dir
    when 0 then dirs.delete(1)
    when 1 then dirs.delete(0)
    when 2 then dirs.delete(3)
    when 3 then dirs.delete(2)
    end
...
AdventureTileEffects::MapTile.add(:Teleporter,
  proc { |id, tile, adventure, scene, dir, dirs|
    ...
    scene.map_sprites.each_value do |sprite|
      ...
      visited = (sprite.visited?) ? 0 : 1
      crossroads[visited].push(sprite)
    end
    if crossroads[0].length >= 1
      scene.pbAutoPosition(scene.player_tile, 6)
      ...
      crossroads[1].each { |sprite| sprite.color = Color.new(255, 0, 0, 125) }
      scene.pbFreeMapScrolling(true)
      crossroads[1].each { |sprite| sprite.color = Color.new(0, 0, 0, 0) }
      scene.pbHideUI
...
AdventureTileEffects::MapTile.add(:Berries,
  proc { |id, tile, adventure, scene, dir, dirs|
    ...
    if needs_healing || adventure.playtesting
      pbMessage(_INTL("\\se[Anim/Recovery]Your Pokémon ate the berries and some of their HP & PP was restored!")) { scene.pbUpdate }
      if !adventure.playtesting
        $player.party.each do |pkmn|
          pkmn.hp += pkmn.totalhp / 2
          pkmn.hp = pkmn.totalhp if pkmn.hp > pkmn.totalhp
          pkmn.moves.length.times do |i|
            pbRestorePP(pkmn, i, 5)
          end
        end
      end
      tile.deactivate
...
AdventureTileEffects::MapTile.add(:Assistant,
  proc { |id, tile, adventure, scene, dir, dirs|
    ...
    if !adventure.playtesting
      ...
      msgs[2..3].each { |msg| pbMessage(msg) { scene.pbUpdate } }
    end
    tile.deactivate
    next dir
  }
...
AdventureTileEffects::MapTile.add(:MysteryNPC,
  proc { |id, tile, adventure, scene, dir, dirs|
    tiles = []
    GameData::AdventureTile.each_type(_INTL("Character")) do |tile|
      next if tile.id == id
      next if tile.partner
      next if tile.styles.length > 0 && !tile.styles.include?(adventure.style)
      tiles.push(tile.id)
    end
    AdventureTileEffects.triggerTile(tiles.sample, tile, adventure, scene, dir, dirs)
    tile.deactivate
    next dir
  }
)
    ...
    if !adventure.playtesting
      ...
      when :Max
        msgs = [_INTL("#{g}Hiya! I've been conducting my research here to uncover the secrets of Dynamax."),
                _INTL("#{g}I've discovered techniques that allows me to increase the Dynamax level of any Pokémon at will!"),
      ...
      when :Tera
        ...
        msgs = [_INTL("#{g}Hiya! I've been conducting my research here to uncover the secrets of Pokémon abilities."),
                _INTL("#{g}I've discovered techniques that allows me to change the abilities of any Pokémon at will!"),
                _INTL("#{g}At least, I think I have...let me practice on your-\nEhem...I mean, let me change the abilities of your Pokémon!")]
      end
      msgs.each { |msg| pbMessage(msg) { scene.pbUpdate } }
      case adventure.style
      ...
      else             pbFadeOutIn { pbAdventureMenuAbilities }
      end
      msgs = [_INTL("#{g}Phew...hey! Nothing exploded this time!\\wt[20] ...Huh?\\wt[20]\nOh, erm...nevermind that."),
              _INTL("#{g}Thanks a lot for helping me further my research! I must go now and report my findings!")]
      msgs.each { |msg| pbMessage(msg) { scene.pbUpdate } }
    end
    tile.deactivate
    next dir
