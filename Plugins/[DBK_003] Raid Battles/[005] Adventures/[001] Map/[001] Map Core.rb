...
class AdventureMapScene
  ...
  def pbSetupBattles
    ...
    base_rules = {
      ...
      :turn_count => 15,                # The max number of turns for each raid battle.
    ...
    @adventure.raid_species.each do |rank, ids|
      ids.each do |sp|
        ...
        rules[:rank] = rank
        species = pbDefaultRaidSpecies(sp, rules)
        level = [($player.badge_count + 1) * 7, 56].min
        level += 4 if rank == 6
  ...
  def pbUpdateHearts(value = nil, addToMax = nil)
    if value
      if addToMax
        ...
        @adventure.hearts = 6 if @adventure.hearts > 6
  ...
  def pbEndScene
    ...
    @viewport2.dispose
    if !@adventure.playtesting
      case @adventure.outcome
      when 1  # Victory - Full rewards.
        @adventure.finalize_loot
        if !@adventure.captures.empty?
          pbFadeOutIn(99999, true) { 
            pbAdventureMenuReward
            pbFadeOutIn { pbAdventureMenuSpoils } if !@adventure.loot.empty?
          }
        elsif !@adventure.loot.empty?
          pbFadeOutIn(99999, true) { pbAdventureMenuSpoils }
        end
      when 3  # Defeat vs. raid boss - Partial rewards.
        if !@adventure.loot.empty?
          pbFadeOutIn(99999, true) { pbAdventureMenuSpoils }
        end
      end
      pbBGMFade(1.0)
