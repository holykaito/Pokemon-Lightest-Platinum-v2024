...
class Battle::AI
  ...
  def choose_cheer_to_use
    ...
    if @trainer.medium_skill?
      ...
      if max_score <= CHEER_USELESS_SCORE
        badCheers = true
      elsif max_score < CHEER_BASE_SCORE + 20
  ...
  #-----------------------------------------------------------------------------
  # Used to score inverted stat stages when using the Lv.1 "Turn the Tables" cheer.
  #-----------------------------------------------------------------------------
  def get_score_for_stat_change(score, target, num_battlers, stat_changes, inverse_count = 0, raising_stats = true)
    desire_mult = (target.opposes?(@user)) ? -1 : 1
    desire_mult *= -1 if !raising_stats
    if stat_changes.length == 0
      mod = (inverse_count > 0) ? 10 : 5
      score -= mod * desire_mult
      return score
    end
    ...
    if !raising_stats || !target.has_move_with_function?("PowerHigherWithUserPositiveStatStages")
      foe_is_aware = false
      each_foe_battler(target.side) do |b, i|
        foe_is_aware = true if !b.has_active_ability?(:UNAWARE)
      end
      score -= 10 if !foe_is_aware
    end
    worthwhile_stats = []
    stat_changes.each_with_index do |stat, idx|
      next if idx.odd?
      if raising_stats
        worthwhile = stat_raise_worthwhile?(target, stat, true)
      else
        worthwhile = stat_drop_worthwhile?(target, stat, true)
      end
      if worthwhile
        worthwhile_stats.push([stat, stat_changes[idx + 1]])
      else
        score -= 5 * desire_mult
      end
    end
    return score if worthwhile_stats.length == 0
    worthwhile_stats.each do |change|
      if raising_stats
        mod = get_target_stat_raise_score_one(score, target, change[0], change[1], desire_mult)
      else
        mod = get_target_stat_drop_score_one(score, target, change[0], change[1], desire_mult)
      end
      score += mod * desire_mult
    end
    total_changes = worthwhile_stats.sum { |change| change[1] }
    if @trainer.has_skill_flag?("HPAware")
      if target.opposes?(@user) && !raising_stats
        score += total_changes.abs * ((100 * target.hp / target.totalhp) - 50) / 8
      elsif !target.opposes?(@user) && raising_stats
        score += total_changes.abs * ((100 * target.hp / target.totalhp) - 50) / 8
      end
    end
    return (score / num_battlers).round
  end
end