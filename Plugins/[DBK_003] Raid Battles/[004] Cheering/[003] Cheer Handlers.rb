...
# Cheer Lv.3 : Heals the team by 100%, removes harmful conditions, and restores half max PP to each move.
#-------------------------------------------------------------------------------
Battle::Cheer::CheerEffects.add(:Healing,
  proc { |cheer, side, owner, battler, battle|
    ...
    battle.allSameSideBattlers(battler).each do |b|
      ...
      if cheerLvl == 3 && b.pokemon.moves.any? { |m| m.pp < m.total_pp }
        b.pokemon.moves.length.times do |i|
        totalPP = b.pokemon.moves[i].total_pp
        pbBattleRestorePP(b.pokemon, b, i, [(totalPP / 2).ceil, 1].max)
	    end
        battle.pbDisplay(_INTL("{1}'s PP was restored.", b.pbThis))
      ret = true
      end
    end
    next ret
  ...
  proc { |cheer, side, owner, battler, battle|
    ...
    when 3
      ...
      battle.allOtherSideBattlers(battler).each do |b|
        next if b.effects[PBEffects::HealBlock] > 0
        b.effects[PBEffects::HealBlock] = 5
...
Battle::Cheer::CheerEffects.add(:BasicRaid,
  proc { |cheer, side, owner, battler, battle|
    ...
    when 3
      ...
      battle.pbRaidChangeTurnCount(foe, count)
      battle.pbDisplay(_INTL("The cheer increased the number of remaining turns!"))
      max_hearts = pbInRaidAdventure? && battle.raidRules[:ko_count] >= 6
      if !max_hearts
