...
class Window_Quest < Window_DrawableCommand
  ...
  def drawItem(index,_count,rect)
    ...
    drawFormattedTextEx(self.contents,rect.x,rect.y+3,
...
class QuestList_Scene
  ...
  def pbStartScene
    ...
    @viewport.z = 99999
    @sprites = {}
    @base = Color.new(50,50,58)
    @shadow = Color.new(200,200,208)
    ...
    @quests_text = ["ĐANG TIẾN HÀNH", "HOÀN THÀNH"]
    if SHOW_FAILED_QUESTS
      @quests.push($PokemonGlobal.quests.failed_quests)
      @quests_text.push("THẤT BẠI")
    end
	###
	if SORT_QUESTS
	  @quests.each do |s|
    ...
    pbDrawTextPositions(@sprites["overlay1"].bitmap,[
      [_INTL("NHIỆM VỤ {1}", @quests_text[@current_quest]),6,9,0,Color.new(248,248,248),Color.new(0,0,0),true]
    ])
    drawFormattedTextEx(@sprites["overlay_control"].bitmap,38,322,
      436,"<c2=#{colorQuest("orange")}>Mũi tên:</c2> Điều hướng",@base,@shadow)
    drawFormattedTextEx(@sprites["overlay_control"].bitmap,38,354,
      436,"<c2=#{colorQuest("orange")}>A/S:</c2> Kéo Xuống/Lên",@base,@shadow)
    drawFormattedTextEx(@sprites["overlay_control"].bitmap,320,322,
      436,"<c2=#{colorQuest("orange")}>Hoạt động mới:</c2>",@base,@shadow)
    pbDrawImagePositions(@sprites["overlay_control"].bitmap,[
      [sprintf("Graphics/UI/QuestUI/new"),465,314]
    ])
  ...
  def drawQuestDesc(quest)
    ...
    pbDrawTextPositions(@sprites["overlay2"].bitmap,[
      ["#{questName}",6,9,0,Color.new(248,248,248),Color.new(0,0,0),true]
    ])
    # Quest description
    ...
    drawFormattedTextEx(@sprites["overlay3"].bitmap,38,320,
      436,"<c2=#{colorQuest("orange")}>Công việc:</c2> #{questStageDesc}",@base,@shadow)
    drawFormattedTextEx(@sprites["overlay3"].bitmap,38,352,
      436,"<c2=#{colorQuest("purple")}>Địa điểm:</c2> #{questStageLocation}",@base,@shadow)
  end
  ...
  def drawOtherInfo(quest)
    ...
    textpos = [
      [sprintf("GIAI ĐOẠN %d/%d",quest.stage,questLength),38,53,0,@base,@shadow],
      ["#{questGiver}",38,125,0,@base,@shadow],
      ["#{originalMap}",38,197,0,@base,@shadow],
      ["#{time}",38,269,0,@base,@shadow]
    ]
### Code for Percy
...
###
    drawFormattedTextEx(@sprites["overlay3"].bitmap,38,92,
      436,"<c2=#{colorQuest("blue")}>Nhận từ:</c2>",@base,@shadow)
    drawFormattedTextEx(@sprites["overlay3"].bitmap,38,164,
      436,"<c2=#{colorQuest("blue")}>Được phát hiện #{loc}:</c2>",@base,@shadow)
    drawFormattedTextEx(@sprites["overlay3"].bitmap,38,236,
      436,"<c2=#{colorQuest("blue")}>Thời gian #{time_text}:</c2>",@base,@shadow)
    drawFormattedTextEx(@sprites["overlay3"].bitmap,38,Graphics.height-61,
      436,"<c2=#{colorQuest("red")}>Phần thưởng:</c2> #{questReward}",@base,@shadow)
