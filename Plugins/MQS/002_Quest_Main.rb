...
class Player_Quests
  ...
  def activateQuest(quest,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        pbMessage("Bạn đã bắt đầu nhiệm vụ này rồi.")
        return
      end
    end
    for i in 0...@completed_quests.length
      if @completed_quests[i].id == quest
        pbMessage("Bạn đã hoàn thành nhiệm vụ này rồi.")
        return
      end
    end
    for i in 0...@failed_quests.length
      if @failed_quests[i].id == quest
        pbMessage("Bạn đã thất bại trong nhiệm vụ này rồi.")
        return
      end
    end
    @active_quests.push(Quest.new(quest,color,story))
    pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("magenta")}>Nhiệm vụ mới</c2>\nXem nhật ký nhiệm vụ để biết thêm chi tiết!</ac>",QUEST_JINGLE))
  end #New quest discovered!
  ...
  def failQuest(quest,color,story)
    ...
    for i in 0...@completed_quests.length
      if @completed_quests[i].id == quest
        pbMessage("Bạn đã hoàn thành nhiệm vụ này.")
        return
      end
    end
    for i in 0...@failed_quests.length
      if @failed_quests[i].id == quest
        pbMessage("Bạn đã thất bại trong nhiệm vụ này.")
        return
      end
    end 
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        found = true
        pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>Nhiệm vụ thất bại!</c2>\nNhật ký nhiệm vụ đã được cập nhật!</ac>",QUEST_FAIL))
        break
      end
    end
    if !found
  ...
  def completeQuest(quest,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        @active_quests.delete_at(i)
        found = true
        pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("green")}>Nhiệm vụ hoàn thành!</c2>\nNhật ký nhiệm vụ đã được cập nhật!</ac>",QUEST_JINGLE))
        break
      end
    end
    if !found
  ...
  def advanceQuestToStage(quest,stageNum,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        found = true
        pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>Đã thêm công việc mới!</c2>\nNhật ký nhiệm vụ đã được cập nhật!</ac>",QUEST_JINGLE))
      end
      return if found
    end
    if !found
...
def taskCompleteJingle
  pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>Công việc hoàn thành!</c2>\nNhật ký nhiệm vụ đã được cập nhật!</ac>",QUEST_JINGLE))
end