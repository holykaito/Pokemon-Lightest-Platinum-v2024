...
class Battle::Scene  
  ...
  def pbUpdateMoveInfoWindow(battler, specialAction, cw)
    ...
    textPos.push(
      [move.name,       xpos + 10,  ypos + 16, :left,   BASE_LIGHT, SHADOW_LIGHT, :outline],
      [_INTL("Pow"),    xpos + 256, ypos + 44, :left,   BASE_LIGHT, SHADOW_LIGHT],
      [displayPower,    xpos + 309, ypos + 44, :center, powBase,    powShadow],
      [_INTL("Acc"),    xpos + 348, ypos + 44, :left,   BASE_LIGHT, SHADOW_LIGHT],
      [displayAccuracy, xpos + 401, ypos + 44, :center, accBase,    accShadow],
      [_INTL("Pri"),    xpos + 442, ypos + 44, :left,   BASE_LIGHT, SHADOW_LIGHT],
      [displayPriority, xpos + 484, ypos + 44, :center, priBase,    priShadow],
      [_INTL("Eff"),    xpos + 428, ypos + 16, :left,   BASE_LIGHT, SHADOW_LIGHT],
      [displayChance,   xpos + 484, ypos + 16, :center, effBase,    effShadow]
    )
    textPos.push([bonus[0], xpos + 8, ypos + 136, :left, bonus[1], bonus[2], :outline]) if bonus
    pbDrawTextPositions(@enhancedUIOverlay, textPos)
    drawTextEx(@enhancedUIOverlay, xpos + 8, ypos + 78, Graphics.width - 12, 2, 
