...
class Battle::Scene
  ...
  def pbUpdateBallSelection(items, index, showDesc = false)
    ...
    textY = (showDesc) ? ypos - 55 : ypos + 18
    action = (showDesc) ? _INTL("Z: Ẩn") : _INTL("Z: Chi tiết")
    item = GameData::Item.try_get(items[index][0])
    name = (item) ? _INTL("{1}", item.name) : _INTL("Trở về")
    desc = (item) ? item.description : _INTL("Quay lại menu.")
    textPos = [
      [_INTL("C: Dùng"), 46, textY, :center, BASE_LIGHT],
