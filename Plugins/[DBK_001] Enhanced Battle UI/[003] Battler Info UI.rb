...
class Battle::Scene
  ...
  def pbUpdateBattlerInfo(battler, effects, idxEffect = 0)
    ...
    textPos  = [
      [_INTL("{1}", poke.name), iconX + 82, iconY - 16, :center, BASE_DARK, SHADOW_DARK],
      [_INTL("{1}", level), xpos + 38, ypos + 108, :left, BASE_LIGHT, SHADOW_LIGHT],
      [_INTL("Đã dùng: {1}", movename), xpos + 342, ypos + 108, :center, BASE_LIGHT, SHADOW_LIGHT],
      [_INTL("Lượt {1}", @battle.turnCount + 1), Graphics.width - xpos - 32, ypos + 12, :center, BASE_DARK, SHADOW_DARK]
    ]
    #---------------------------------------------------------------------------
    # Battler icon.
    ...
    if battler.pbOwnedByPlayer?
      ...
      textPos.push(
        [_INTL("Abil."), xpos + 272, ypos + 48, :center, BASE_LIGHT, SHADOW_LIGHT],
        [_INTL("Item"), xpos + 272, ypos + 72, :center, BASE_LIGHT, SHADOW_LIGHT],
    ...
    [[:ATTACK,          _INTL("Attack")],
     ...
     _INTL("Crit. Hit")
      ...
      if stat.is_a?(Array)
        ...
        textPos.push([stat[1], xpos + 16, ypos + 142 + (i * 24), :left, BASE_LIGHT, color])
        stage = battler.stages[stat[0]]
      else
        textPos.push([stat, xpos + 16, ypos + 142 + (i * 24), :left, BASE_LIGHT, SHADOW_LIGHT])
  ...
  def pbGetDisplayEffects(battler)
    ...
    if battler.damageThreshold
      desc = _INTL("HP của Pokémon sẽ không giảm xuống dưới {1}% khi bị tấn công.", battler.damageThreshold)
    ...
    if battler.dynamax?
      ...
      desc = _INTL("Pokémon đang ở trạng thái Dynamax.")
      display_effects.push([_INTL("Dynamax"), tick, desc])
    elsif battler.tera?
      data = GameData::Type.get(battler.tera_type).name
      desc = _INTL("Pokémon này được Terastallized thành hệ {1}.", data)
    ...
    if weather != :None
      if weather == :Hail
        if defined?(Settings::HAIL_WEATHER_TYPE) && Settings::HAIL_WEATHER_TYPE > 0
          ...
          when 1
            name = _INTL("Snow")
            desc = _INTL("Tăng Def của hệ Ice. Blizzard luôn trúng.")
          when 2
            name = _INTL("Hailstorm")
            desc = _INTL("Kết hợp hiệu ứng của Hail và Snow.")
          end
        else
          name = GameData::BattleWeather.get(weather).name
          desc = _INTL("Không phải hệ Ice sẽ chịu sát thương mỗi lượt. Blizzard luôn trúng.")
        end
      else
        name = GameData::BattleWeather.get(weather).name
        case weather
        when :Sun         then desc = _INTL("Đòn hệ Fire mạnh lên và đòn hệ Water yếu đi.")
        when :HarshSun    then desc = _INTL("Đòn hệ Fire mạnh lên và đòn hệ Water yếu đi.")
        when :Rain        then desc = _INTL("Đòn hệ Water mạnh lên và đòn hệ Fire yếu đi.")
        when :HeavyRain   then desc = _INTL("Đòn hệ Water mạnh lên và đòn hệ Fire yếu đi.")
        when :Snow        then desc = _INTL("Tăng Def của hệ Ice. Blizzard luôn trúng.")
        when :Sandstorm   then desc = _INTL("Tăng Sp.Def của hệ Rock. Chịu dame nếu không phải hệ Rock/Ground/Steel.")
        when :StrongWinds then desc = _INTL("Hệ Flying không có điểm yếu")
        when :ShadowSky   then desc = _INTL("Đòn bóng tối mạnh hơn. Chịu dame nếu không phải là Shadow")
        else                   desc = _INTL("Không rõ thời tiết")
        end
      end
