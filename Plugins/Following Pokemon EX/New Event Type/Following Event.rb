...
class Game_FollowingPkmn < Game_Follower
  ...
  def screen_x(*args)
    ...
    case self.direction
    when 4 then ret += offset  # Facing Left -> Move Right
    when 6 then ret -= offset  # Facing Right -> Move Left
    end
    return ret
  end
  ...
  def screen_y(*args)
    ...
    # Move opposite to facing direction to "push away" from what it's facing (usually player)
    case self.direction
    when 2 then ret -= offset  # Facing Down -> Move Up
    when 8 then ret += offset  # Facing Up -> Move Down
    end
    return ret
  end
  #-----------------------------------------------------------------------------
