...
class Battle::AI::AIBattler
  ...
  def rough_end_of_round_damage
    ...
    when :ICEBODY
      ret -= [self.totalhp / 12, 1].max if weather == :Hail && battler.canHeal?
    when :RAINDISH
      ret -= [self.totalhp / 12, 1].max if [:Rain, :HeavyRain].include?(weather) && battler.canHeal?
    when :SOLARPOWER
      ret += [self.totalhp / 8, 1].max if [:Sun, :HarshSun].include?(weather) && battler.takesIndirectDamage?
    end
  ...
  def effectiveness_of_type_against_single_battler_type(type, defend_type, user = nil)
    ...
    if Effectiveness.ineffective_type?(type, defend_type)
      ...
      # Ground Shock
      if user&.has_active_ability?(:GROUNDSHOCK) && defend_type == :GROUND
        ret = Effectiveness::NORMAL_EFFECTIVE_MULTIPLIER
      end
      # Miracle Eye
