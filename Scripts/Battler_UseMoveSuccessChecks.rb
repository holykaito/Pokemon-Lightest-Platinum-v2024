class Battle::Battler
  ...
  def pbSuccessCheckAgainstTarget(move, user, target, targets)
    ...
    if move.damagingMove? && move.calcType == :GROUND &&
      ...
      if target.hasActiveAbility?(:BRANCHSWING) && @battle.field.terrain == :Grassy && target.affectedByTerrain?
        # Check if Mold Breaker is not in effect
        unless @battle.moldBreaker
          # Display the ability splash and the message
          if show_message
            @battle.pbShowAbilitySplash(target)
            if Battle::Scene::USE_ABILITY_SPLASH
              @battle.pbDisplay(_INTL("{1} avoided the attack!", target.pbThis))
          ...
          # Indicate that the move should not hit the target
          return false
        end
      end
