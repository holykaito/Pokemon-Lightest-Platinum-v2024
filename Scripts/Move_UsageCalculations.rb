class Battle::Move
  ...
  def pbCalcTypeModSingle(moveType, defType, user, target)
    ...
    if Effectiveness.ineffective_type?(moveType, defType)
      ...
      # Ground Shock
      if user.hasActiveAbility?(:GROUNDSHOCK) && defType == :GROUND
        ret = Effectiveness::NORMAL_EFFECTIVE_MULTIPLIER
      end
      # Miracle Eye
  ...
  def pbCalcDamage(user, target, numTargets = 1)
    ...
    #Spell Fist Damage calc
    if user.hasActiveAbility?(:SPELLFIST) && move.punchingMove?
      # Temporarily change the move's category to special
      old_category = move.category
      move.category = 1 if move.category == 0  # Change to special if it's physical
      # Call the original damage calculation method
      damage = original_pbCalcDamage(move, user, target, numTargets)
      # Restore the move's original category
      move.category = old_category
      return damage
    end
    # Calculate all multiplier effects
    multipliers = {
  ...
  def pbCalcDamageMultipliers(user, target, numTargets, type, baseDmg, multipliers)
    ...
    when :Misty
      multipliers[:power_multiplier] *= terrain_multiplier if type == :FAIRY && user.affectedByTerrain?
    ...
    if target.damageState.critical
      ...
      if user.hasActiveAbility?(:CATSLUCK)
       # Array of all stats
       stats = [:ATTACK, :DEFENSE, :SPEED, :SPATK, :SPDEF, :EVASION, :ACCURACY]
       # Choose a random stat
       random_stat = stats.sample
       # Increase the chosen stat by 1 stage
       if user.pbCanRaiseStatStage?(random_stat, user)
        user.pbRaiseStatStage(random_stat, 1, user)
        user.pbDisplay(_INTL("{1}'s {2} raised its {3} due to Cat's Luck!", 
        user.pbThis, PBAbilities.getName(user.ability), 
        PBStats.getName(random_stat)))
       end
      end
    end
    # Random variance
