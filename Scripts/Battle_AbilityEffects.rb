...
module Battle::AbilityEffects
  ...
  # New
  OnMoveUseFromUser                = AbilityHandlerHash.new
  MoveUseFromUser                  = AbilityHandlerHash.new
  ...
  def self.triggerOnMoveUseFromUser(ability, battler)
    return trigger(OnMoveUseFromUser, ability, user, targets, move, battle)
  end
  
  def self.triggerMoveUseFromUser(ability, battler)
    return trigger(MoveUseFromUser, ability, user, targets, move, battle)
  end
end
...
Battle::AbilityEffects::SpeedCalc.add(:BRANCHSWING,
  proc { |ability, battler, mult|
    next mult * 1.5 if battler.battle.field.terrain == :Grassy
  }
)
...
Battle::AbilityEffects::OnStatLoss.add(:FORTITUDE,
  proc { |ability, battler, stat, user|
    next if user && !user.opposes?(battler)
...
Battle::AbilityEffects::AccuracyCalcFromUser.add(:ECHOLOCK,
  proc { |ability, mods, user, target, move, type|
    mods[base_accuracy] = 0 if move.soundMove?
  }
)

Battle::AbilityEffects::AccuracyCalcFromTarget.add(:PHANTOMIMAGE,
  proc { |ability, mods, user, target, move, type|
    if target.speed>=user.speed*2
      mods[:evasion_multiplier] *= 2
    elsif target.speed>=user.speed*1.5
      mods[:evasion_multiplier] *= 1.5
...
Battle::AbilityEffects::AccuracyCalcFromTarget.add(:DODGE,
  proc { |ability, mods, user, target, move, type|
    mods[:evasion_multiplier] *= 1.15
  }
)

Battle::AbilityEffects::AccuracyCalcFromTarget.add(:HUNTEDONE,
  proc { |ability, mods, user, target, move, type|
    mods[:evasion_multiplier] *= 0.8
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:AERILATE,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.3 if move.powerBoost
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:AQUATIC,
  proc { |ability, user, target, move, mults, power, type|
    mults[:attack_multiplier] *= 1.25 if type == :WATER
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:BLAZE,
  proc { |ability, user, target, move, mults, power, type|
    ...
    elsif user.hp <= user.totalhp / 2 && type == :FIRE
      mults[:attack_multiplier] *= 1.2
    elsif user.hp <= user.totalhp / 1 && type == :FIRE
...
Battle::AbilityEffects::DamageCalcFromUser.add(:FIELDEXPLORER,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.3 if move.fieldMove?
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:SPELLFIST,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.2 if move.punchingMove?
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:STRIKER,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.2 if move.kickMove?
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:PACKLEADER,
  proc { |ability, user, target, move, mults, power, type|
    next false if user.battle.pbAbleCount(user) < 3
    next false if user.battle.pbTeamLengthFromBattlerIndex(user) < 3
...
Battle::AbilityEffects::DamageCalcFromUser.add(:OVERGROW,
  proc { |ability, user, target, move, mults, power, type|
    if user.hp <= user.totalhp / 3 && type == :GRASS
      mults[:attack_multiplier] *= 1.5
    elsif user.hp <= user.totalhp / 2 && type == :GRASS
      mults[:attack_multiplier] *= 1.2
    elsif user.hp <= user.totalhp / 1 && type == :GRASS
      mults[:attack_multiplier] *= 1.1
    end
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:ICYVEINS,
  proc { |ability, user, target, move, mults, power, type|
    if user.effectiveWeather == :Hail &&
       [:ICE, :WATER].include?(type)
      mults[:power_multiplier] *= 1.3
    end
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:SPELLCASTER,
  proc { |ability, user, target, move, mults, power, type|
    mults[:attack_multiplier] *= 1.2 if type == :ICE || type == :FIRE || type == :ELECTRIC
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:JUNGLECOMBAT,
  proc { |ability, user, target, move, mults, power, type|
    mults[:final_damage_multiplier] *= 2 if Effectiveness.resistant?(target.damageState.typeMod) && type == :GRASS
  }
)
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:COTTONCLOAK,
  proc { |ability, user, target, move, mults, power, type|
    mults[:defense_multiplier] *= 2 if move.physicalMove? ||
                                       move.function_code == "UseTargetDefenseInsteadOfTargetSpDef"   # Psyshock
  }
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:SHIELDDUST,
  proc { |ability, user, target, move, mults, power, type|
    mults[:defense_multiplier] *= 1.25 if move.pbContactMove?(user)
  }
)
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:PRIDEFULGUARD,
  proc { |ability, user, target, move, mults, power, type|
    mults[:final_damage_multiplier] /= 2 if target.hp == target.droppedBelowHalfHP
  }
)
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:AQUATIC,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] /= 1.5 if [:WATER, :ICE, :FIRE, :STEEL].include?(type)
    mults[:power_multiplier] *= 1.5 if [:ELECTRIC, :GRASS].include?(type)
  }
)

Battle::AbilityEffects::DamageCalcFromTarget.add(:INFLATABLE,
  proc { |ability, user, target, move, mults, power, type|
    mults[:final_damage_multiplier] /= 2 if type == :FIRE || move.windMove?
  }
)
...
Battle::AbilityEffects::OnBeingHit.add(:INFLATABLE,
  proc { |ability, user, target, move, battle|
    next if ![:FIRE].include?(move.calcType) || !move.windMove?
...
Battle::AbilityEffects::OnBeingHit.add(:WILDTANTRUM,
  proc { |ability, user, target, move, battle|
    next if battle.pbRandom(100) >= 30
    target.pbRaiseStatStageByAbility(:ATTACK, 1, target)
  }
...
Battle::AbilityEffects::OnBeingHit.add(:SPOREVEIL,
  proc { |ability, user, target, move, battle|
...
Battle::AbilityEffects::OnDealingHit.add(:PIERCEARMOR,
  proc { |ability, user, target, move, battle|
    next if !move.contactMove?
    battle.pbShowAbilitySplash(user)
    if target.hasActiveAbility?(:SHIELDDUST) && !battle.moldBreaker
      ...
      battle.pbHideAbilitySplash(target)
    elsif target.pbCanLowerStatStage?(:DEFENSE, user, self)
...
Battle::AbilityEffects::OnDealingHit.add(:MINUS,
  proc { |ability, user, target, move, battle|
...
Battle::AbilityEffects::OnDealingHit.add(:PLUS,
  proc { |ability, user, target, move, battle|
    next if move.calcType != :ELECTRIC
    next if battle.pbRandom(100) >= 15
    battle.pbShowAbilitySplash(user)
...
Battle::AbilityEffects::OnDealingHit.add(:VAMPIRIC,
  proc { |ability, user, target, move, battle|
...
Battle::AbilityEffects::OnDealingHit.add(:GROWINGTOOTH,
  proc { |ability, user, target, move, battle|
...
Battle::AbilityEffects::OnDealingHit.add(:SEEDSTIMULUS,
...
Battle::AbilityEffects::OnEndOfUsingMove.add(:HUBRIS,
  proc { |ability, user, targets, move, battle|
    next if battle.pbAllFainted?(user.idxOpposingSide)
    numFainted = 0
    targets.each { |b| numFainted += 1 if b.damageState.fainted }
...
Battle::AbilityEffects::AfterMoveUseFromTarget.add(:SHOWDOWNMODE,
  proc { |ability, target, user, move, switched_battlers, battle|
    next if !move.damagingMove?
    next if !target.droppedBelowHalfHP
    if [:ATTACK, :DEFENSE, :SPEED, :SPECIAL_ATTACK, :SPECIAL_DEFENSE].any? { |stat| target.pbCanRaiseStatStage?(stat, target) }
      show_message = false
      stat_list = [:ATTACK, :DEFENSE, :SPEED, :SPECIAL_ATTACK, :SPECIAL_DEFENSE]
      stat_list.each do |stat|
        if target.pbCanRaiseStatStage?(stat, target)
          target.pbRaiseStatStageByAbility(stat, 1, target, true)
          show_message = true
        end
      end
      if show_message
        battle.pbDisplay(_INTL("{1}'s stats rose due to its Showdown Mode!", target.pbThis))
      end
      target.effects[PBEffects::ShowdownMode] = true
    end
  }
)
...
Battle::AbilityEffects::EndOfRoundWeather.add(:ICEBODY,
  proc { |ability, weather, battler, battle|
    ...
    next if !battler.canHeal?
    battle.pbShowAbilitySplash(battler)
    battler.pbRecoverHP(battler.totalhp / 12)
    if Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("{1}'s HP was restored.", battler.pbThis))
    else
      battle.pbDisplay(_INTL("{1}'s {2} restored its HP.", battler.pbThis, battler.abilityName))
...
Battle::AbilityEffects::EndOfRoundEffect.add(:SELFSUFFICIENT,
  proc { |ability, battler, battle|
    next if !battler.canHeal?
    next if !Battler.droppedBelowHalfHP
    battle.pbShowAbilitySplash(battler)
...
Battle::AbilityEffects::EndOfRoundEffect.add(:BATTLEINTELLECT,
  proc { |ability, battler, battle|
    battle.pbPriority(true).each do |b|
      next if b == battler  # Skip the user of the ability
      next if b.unstoppableAbility?  # Skip if the target has an unstoppable ability
      next if b.ability == :HUNTEDONE  # Skip if the target already has the "Hunted One" ability
      
      if battle.pbRandom(100) < 30  # 30% chance
        oldAbil = b.ability
        battle.pbShowAbilitySplash(battler, true, false)
        b.ability = :HUNTEDONE
        battle.pbReplaceAbilitySplash(b) if b.opposes?(battler)
        if Battle::Scene::USE_ABILITY_SPLASH
          battle.pbDisplay(_INTL("{1}'s Ability became {2}!", b.pbThis, b.abilityName))
        else
          battle.pbDisplay(_INTL("{1}'s Ability became {2} because of {3}!",
             b.pbThis, b.abilityName, battler.pbThis(true)))
        end
        battle.pbHideAbilitySplash(b) if b.opposes?(battler)
...
Battle::AbilityEffects::EndOfRoundEffect.add(:SHOWDOWNMODE,
  proc { |ability, battler, battle|
    battle.eachBattler do |battler|
      next if !battler.effects[PBEffects::ShowdownMode]
      next if battler.hp == 0
      next if battler.hp <= battler.totalhp / 18  # Ensure battler does not faint
      battle.pbDisplay(_INTL("{1} is losing health due to Showdown Mode!", battler.pbThis))
      battler.pbReduceHP(battler.totalhp / 18)
      battler.pbItemHPHealCheck
    end
  }
)

Battle::AbilityEffects::EndOfRoundEffect.add(:RESILIENCE,
  proc { |ability, battler, battle|
    next if battler.hp > battler.totalhp / 3
    next if !battler.canHeal?
    battle.pbShowAbilitySplash(battler)
    battler.pbRecoverHP(battler.totalhp / 8)
    if Battle::Scene::USE_ABILITY_SPLASH
...
Battle::AbilityEffects::EndOfRoundEffect.add(:HEADACHE,
  proc { |ability, battler, battle| 
    next if battler.effects[PBEffects::Confusion] == 0
...
Battle::AbilityEffects::OnSwitchIn.add(:ROLLER,
  proc { |ability, battler, battle, switch_in|
    battler.pbRaiseStatStageByAbility(:DEFENSE, 1, battler)
    battler.effects[PBEffects::DefenseCurl] = true
  }
)

Battle::AbilityEffects::OnSwitchIn.add(:INFLATABLE,
  proc { |ability, battler, battle, switch_in|
    next if battler.pbOwnSide.effects[PBEffects::Tailwind] <= 0
    next if !battler.pbCanRaiseStatStage?(:SPECIAL_DEFENSE, battler)
...
Battle::AbilityEffects::OnSwitchIn.add(:PACKLEADER,
  proc { |ability, battler, battle, switch_in|
    next false if battle.pbAbleCount(battler) < 3
    next false if battle.pbTeamLengthFromBattlerIndex(battler) < 3
...
Battle::AbilityEffects::OnSwitchIn.add(:PHANTOMPIMAGE,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("{1} is moving in an elusive way!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:DANCINGPANIC,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
