...
class Battle::Move::StatUpMove < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    ...
    if pbReverseDanceEffect?(user) # Custom Ability - Dancing Panic
      return !user.pbCanLowerStatStage?(@statUp[0], user, self, true)
    else
  ...
  def pbEffectGeneral(user)
    ...
    if pbReverseDanceEffect?(user) # Custom Ability - Dancing Panic
      user.pbLowerStatStage(@statUp[0], @statUp[1],user)
    else
  ...
  def pbAdditionalEffect(user, target)
    if pbReverseDanceEffect?(user) # Custom Ability - Dancing Panic
      if user.pbCanLowerStatStage?(@statUp[0], user, self)
        user.pbLowerStatStage(@statUp[0], @statUp[1], user)
      end
    else
...
class Battle::Move::MultiStatUpMove < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    ...
    if pbReverseDanceEffect?(user) # Custom Ability - Dancing Panic
      ...
      if failed
        @battle.pbDisplay(_INTL("{1}'s stats won't go any lower!", user.pbThis))
        return true
      end
    else
      (@statUp.length / 2).times do |i|
  ...
  def pbEffectWhenDealingDamage(user, target)
    ...
    if pbReverseDanceEffect?(user) # Custom Ability - Dancing Panic
      (@statDown.length / 2).times do |i|
        next if !user.pbCanRaiseStatStage?(@statDown[i*2], user, self)
        if user.pbRaiseStatStage(@statDown[i*2],@statDown[i*2+1], user, showAnim)
          showAnim = false
        end
      end
    else
...
class Battle::Move::TargetStatDownMove < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    return false if damagingMove?
    if pbReverseDanceEffect?(user) # Custom Ability - Dancing Panic
      return !target.pbCanRaiseStatStage?(@statDown[0], user, self,true)
    else
...
#    return !target.pbCanLowerStatStage?(@statDown[0], user, self, show_message)
  ...
  def pbEffectAgainstTarget(user, target)
    return if damagingMove?
    if pbReverseDanceEffect?(user) # Custom Ability - Dancing Panic
      target.pbRaiseStatStage(@statDown[0], @statDown[1], user)
    else
...
#    target.pbLowerStatStage(@statDown[0], @statDown[1], user)
  ...
  def pbAdditionalEffect(user, target)
    ...
    if pbReverseDanceEffect?(user) # Custom Ability - Dancing Panic
      return if !target.pbCanRaiseStatStage?(@statDown[0], user, self)
      target.pbRaiseStatStage(@statDown[0], @statDown[1], user)
    else
...
class Battle::Move::TargetMultiStatDownMove < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if pbReverseDanceEffect?(user) # Custom Ability - Dancing Panic
      (@statDown.length / 2).times do |i|
        next if !target.pbCanRaiseStatStage?(@statDown[i*2], user, self)
        failed = false
        break
      end
    else
...
#    end
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if pbReverseDanceEffect?(user) # Custom Ability - Dancing Panic
      (@statDown.length / 2).times do |i|
        next if !target.pbCanRaiseStatStage?(@statDown[i*2], user, self)
        if target.pbRaiseStatStage(@statDown[i*2],@statDown[i*2+1], user, showAnim)
          showAnim = false
        end
      end
    else
