...
module PBDayNight
  HOURLY_TONES = [
    Tone.new(  0,   0,   0,  0),   # Night           # Midnight
    Tone.new(  0,   0,   0,  0),   # Night
    Tone.new(  0,   0,   0,  0),   # Night
    Tone.new(  0,   0,   0,  0),   # Night
    Tone.new(  0,   0,   0,  0),   # Night
    ...
    Tone.new(  0,   0,   0,  0),   # Day/afternoon
    Tone.new(  0,   0,   0,  0),   # Day/afternoon
    Tone.new( -5, -30, -20,  0),   # Day/afternoon
    Tone.new( -5, -30, -20,  0),   # Day/evening     # 6PM
    Tone.new(  0,   0,   0,  0),   # Day/evening
    Tone.new(  0,   0,   0,  0),   # Day/evening
    Tone.new(  0,   0,   0,  0),   # Night
    Tone.new(  0,   0,   0,  0),   # Night
    Tone.new(  0,   0,   0,  0)    # Night
  ...
  def self.isNight?(time = nil)
    time = pbGetTimeNow if !time
    return (time.hour >= 18 || time.hour < 5)
  end
  ...
  def self.isAfternoon?(time = nil)
    time = pbGetTimeNow if !time
    return (time.hour >= 11 && time.hour < 15)
  end

  # Returns true if it's the evening.
  def self.isEvening?(time = nil)
    time = pbGetTimeNow if !time
    return (time.hour >= 17 && time.hour < 19)
  end
