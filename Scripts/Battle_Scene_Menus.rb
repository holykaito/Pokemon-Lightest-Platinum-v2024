...
class Battle::Scene::MenuBase
  ...
  TEXT_SHADOW_COLOR = Color.new(180, 180, 180) #(160, 160, 168)
...
class Battle::Scene::CommandMenu < Battle::Scene::MenuBase
  ...
  def initialize(viewport, z)
    ...
    @msgBox = Window_UnformattedTextPokemon.newWithSize(
      "", self.x + 16, self.y + 6, 220, Graphics.height - self.y, viewport
    )
    @msgBox.baseColor   = Color.new(240, 240, 240) 
    @msgBox.shadowColor = Color.new(80, 80, 80)
...
class Battle::Scene::FightMenu < Battle::Scene::MenuBase
  ...
  PP_COLORS = [
    ...
    Color.new(240, 240, 240), Color.new(80, 80, 80),    # Black, more than 1/2 of total PP
#    TEXT_BASE_COLOR, TEXT_SHADOW_COLOR                # Black, more than 1/2 of total PP
  ...
  def initialize(viewport, z)
    ...
    if USE_GRAPHICS
      ...
      @msgBox.shadowColor = Color.new(75, 75, 75)
...
#      pbSetMoveFont(@overlay.bitmap)  
  def refreshButtonNames
    ...
    @buttons.each_with_index do |button, i|
      ...
      moveNameBase = Color.new(50, 50, 58)
      if GET_MOVE_TEXT_COLOR_FROM_MOVE_BUTTON && moves[i].display_type(@battler)
        ...
        moveNameBase = button.bitmap.get_pixel(10, button.src_rect.y + 32)
      end
      textPos.push([moves[i].name, x, y, :center, moveNameBase, Color.new(50, 50, 58)])
    end
...
class Battle::Scene::TargetMenu < Battle::Scene::MenuBase
  ...
  TEXT_SHADOW_COLOR = Color.new(125, 125, 125)
  ...
  def initialize(viewport, z, sideSizes)
    ...
    @overlay.x = self.x
    @overlay.y = self.y
    pbSetMediumFont(@overlay.bitmap)
    addSprite("overlay", @overlay)
    self.z = z
    refresh
  end
  ...
  def refreshButtons
    ...
    @buttons.each_with_index do |button, i|
      ...
      x = button.x - self.x + (button.src_rect.width / 2)
      y = button.y - self.y + 14
      textpos.push([@texts[i], x, y, :center, Color.new(255, 255, 255), Color.new(125, 125, 125)])
    end
    pbDrawTextPositions(@overlay.bitmap, textpos)
  end
