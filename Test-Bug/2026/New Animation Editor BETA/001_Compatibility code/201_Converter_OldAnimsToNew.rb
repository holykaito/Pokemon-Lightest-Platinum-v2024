module AnimationConverter
  ...
  def convert_extracted_old_animations_to_new
    anim_files = []
    BattleAnimationEditor.pbRgssChdir(".") { anim_files.concat(Dir.glob("*.anm")) }
    anim_files.each do |file_path|
      old_anim = load_old_format_extracted_animation(file_path)
      new_anim = convert_old_format_animation_to_new(old_anim)
      next if !new_anim
      GameData::Animation.register(new_anim)
      Compiler.write_battle_animation_file(new_anim[:pbs_path])
    end
  ...
  def load_old_format_extracted_animation(file_path)
    ret = BattleAnimationEditor.loadBase64Anim(IO.read(file_path))
    throw "Bad data" if !ret.is_a?(PBAnimation)
    ret.id = -1 # this is not an RPG Maker XP animation
    BattleAnimationEditor.pbConvertAnimToNewFormat(ret)
    return ret
  end

  def convert_old_format_animation_to_new(anim)
    return if !anim.name || anim.name == "" || anim.length <= 1
    last_move = nil   # For filename purposes
  ...
  def add_frames_to_new_anim_hash(anim, hash)
    ...
    default_frame[99]                    = "Sage/" + anim.graphic
    ...
    anim.length.times do |frame_num|
      ...
      frame.each_with_index do |cel, i|
        ...
        if i > 1 && frame_num > 0 && index_lookup[i] && index_lookup[i] >= 0 &&
          ...
          this_graphic = (cel[AnimFrame::PATTERN] == -1) ? "USER" : (cel[AnimFrame::PATTERN] == -2) ? "TARGET" : "Sage/" + anim_graphic
          this_graphic.gsub!(".", " ")
          this_graphic.gsub!("  ", "")
        ...
        if i == 0
          ...
          when -2   # Target's sprite
            ...
            particle[:graphic] = "Sage/" + anim_graphic
            last_frame[99] = "Sage/" + anim_graphic
          end
        end
    ...
    hash[:particles].each do |particle|
      [:x, :y,
       ...
       :tone_red, :tone_green, :tone_blue, :tone_gray].each do |property|
    ...
    anim.timing.each do |cmd|
      ...
      when 1, 2, 3, 4   # BG graphic (set, move/recolour), FG graphic (set, move/recolour)
        ...
        if cmd.name && cmd.name != ""
          particle[:graphic] = "Sage/" + cmd.name
          added = true
        end
        if cmd.colorRed
          particle[:color_red] ||= []
...
# MenuHandlers.add(:debug_menu, :convert_anims, {
#   "name"        => "Convert extracted animations to PBS files",
#   "parent"      => :data_importer_menu,
#   "description" => "Finds all .anm files in main directory and converts them to animation PBS files.",
#   "effect"      => proc {
#     AnimationConverter.convert_extracted_old_animations_to_new
#   }
# })
