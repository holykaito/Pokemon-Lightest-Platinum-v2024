class Battle::Battler
	...
	def pbUseMove(choice, specialUsage = false)
		...
		# Ball Fetch
		if !@effects[PBEffects::BallFetch] && !user.lastMoveFailed && realNumHits > 0 &&
			!move.snatched && magicCoater < 0 && @battle.pbCheckGlobalAbility(:BALLFETCH) &&
			move.bombMove? && Settings::MAG_BALLFETCH == true
			bomber = []
			@battle.pbPriority(true).each do |b|
				bomber.push(b) if b.index != user.index && b.hasActiveAbility?(:BALLFETCH)
			end
			while bomber.length > 0
				nextUser = bomber.pop
				oldLastRoundMoved = nextUser.lastRoundMoved
				# NOTE: Petal Dance being used because of Dancer shouldn't lock the
					#       Dancer into using that move, and shouldn't contribute to its
				...
				preTarget = user.index if nextUser.opposes?(user) || !nextUser.opposes?(preTarget)
				@battle.pbShowAbilitySplash(nextUser, true)
					@battle.pbDisplay(_INTL("{1} caught the ball attack with {2}!",
					nextUser.pbThis, nextUser.abilityName))
                @battle.pbHideAbilitySplash(nextUser)
				nextUser.effects[PBEffects::BallFetch] = true
				if nextUser.pbCanChooseMove?(move, false)
					PBDebug.logonerr { nextUser.pbUseMoveSimple(move.id, preTarget) }
					nextUser.lastRoundMoved = oldLastRoundMoved
					nextUser.effects[PBEffects::Outrage] = oldOutrage
				...
				nextUser.effects[PBEffects::BallFetch] = false
			end
		end
	end
  ...
  def pbConfusionDamage(msg)
	...
	if self.status == :VERTIGO
     self.hp -= @damageState.hpLost * 1.5
	else
...
alias mag_pbTryUseMove pbTryUseMove
  def pbTryUseMove(choice, move, specialUsage, skipAccuracyCheck)
  case @status
  when :WINDED
    if @battle.pbRandom(100) < 50
      pbContinueStatus
      PBDebug.log("[Move failed] #{pbThis} is winded")
      @lastMoveFailed = true
      return false
    end
  when :PESTER
    if @battle.pbRandom(100) < 25
      @battle.pbDisplay(_INTL("{1} is scratching its itch and can't move!",
					pbThis))
      PBDebug.log("[Move failed] #{pbThis} is pestered")
      @lastMoveFailed = true
      return false
    end
  when :IDOLIZE
      @battle.pbCommonAnimation("Attract", self)
      @battle.pbDisplay(_INTL("{1} is idolizing!", pbThis,))
    ...
    return mag_pbTryUseMove(choice, move, specialUsage, skipAccuracyCheck)
  end
  
alias mag_pbEffectsAfterMove pbEffectsAfterMove
  def pbEffectsAfterMove(user, targets, move, numHits)
    if user.status == :MIGRAINE && move.statusMove?
	pbContinueStatus
	user.pbReduceHP(user.totalhp / 8, false)
	end
  return mag_pbEffectsAfterMove(user, targets, move, numHits)
  end
